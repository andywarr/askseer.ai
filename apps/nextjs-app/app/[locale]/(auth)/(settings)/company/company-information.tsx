...
export default function CompanyInformation({
  ...
  const nameSchema = useMemo(() => z
    .string()
    .trim()
    .min(1, { message: t("companyNameRequired") }), [t]);
  ...
  const nameError = useMemo(
    () =>
      isEditing && isNameChanged && !isNameValid
        ...
        : undefined,
    [isEditing, isNameChanged, isNameValid, parsedName, t],
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(e?.message || t("failedToUpdateCompany"));
            ...
            {isEditing && (
              <div className="flex items-center gap-2">
                ...
                {currentLogoKey && !draftLogoFile && (
                  ...
                  >
                    {removeExistingLogo ? t("undoBtn") : t("removeBtn")}
                  </Button>
                )}
              </div>
            )}
