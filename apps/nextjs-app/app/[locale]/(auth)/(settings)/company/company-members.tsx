    ...
    (userId: string, role: string) => {
      startMembershipTransition(async () => {
        try {
          ...
          toast.error(error?.message || t("failedToUpdateMembership"));
        }
      });
    },
    ...
    (userId: string, canCreatePersonas: boolean) => {
      ...
      startMembershipTransition(async () => {
        try {
          ...
          toast.success(
            canCreatePersonas
              ? t("personaCreationEnabled")
              : t("personaCreationDisabled"),
          );
        } catch (e: unknown) {
          const error = e as Error;
          toast.error(error?.message || t("failedToUpdatePermissions"));
        }
      });
    },
    [companyId, memberList],
    ...
    (canCreatePersonas: boolean) => {
      ...
      startMembershipTransition(async () => {
        try {
          ...
          toast.success(
            canCreatePersonas
              ? t("personaCreationEnabledAll")
              : t("personaCreationDisabledAll"),
          );
        } catch (e: unknown) {
          const error = e as Error;
          toast.error(error?.message || t("failedToUpdatePersonaPermissionsAll"));
        }
      });
    },
    [companyId, memberList],
  ...
  const handleConfirmRemove = useCallback(() => {
    ...
    startRemoveTransition(async () => {
      try {
        ...
        const error = e as Error;
        toast.error(error?.message || t("failedToDeactivateMember"));
      }
    });
  ...
  const handleConfirmActivate = useCallback(() => {
    ...
    startActivateTransition(async () => {
      try {
        ...
        toast.success(t("memberActivated"));
        setActivateTarget(null);
        setOpenDropdownUserId(null);
      } catch (e: unknown) {
        const error = e as Error;
        toast.error(error?.message || t("failedToActivateMember"));
      }
    });
  ...
  const handleConfirmErase = useCallback(() => {
    ...
    startEraseTransition(async () => {
      try {
        ...
        toast.success(t("userDeletedPermanently"));
        setEraseTarget(null);
        setEraseConfirmation("");
        setOpenDropdownUserId(null);
      } catch (e: unknown) {
        const error = e as { message?: string; teams?: { name: string }[]; companies?: { name: string }[] };
        const message = error?.message || t("failedToDeleteUser");
        if (error?.teams && error.teams.length > 0) {
          toast.error(
            `${message}\n\n${t("teamsWithStudies", { teams: error.teams.map((t) => t.name).join(", ") })}`,
            { duration: 6000 },
          );
        } else if (error?.companies && error.companies.length > 0) {
          toast.error(
            `${message}\n\n${t("companiesWhereSoleOwner", { companies: error.companies.map((c) => c.name).join(", ") })}`,
            { duration: 6000 },
          );
        } else {
          toast.error(message);
