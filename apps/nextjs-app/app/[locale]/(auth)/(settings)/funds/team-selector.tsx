...
function formatBalanceLabel(
  ...
  if (showRemaining) {
    return t ? t("teamSelector.remaining") : "remaining";
  }
  return "";
}
...
export const TeamSelector = memo(function TeamSelector({
  ...
  const formatBalance = useCallback((balanceCents: number) => {
  ...
  const displayPlaceholder = placeholder || t("teamSelector.placeholder");
  const displayEmptyPlaceholder =
    emptyPlaceholder || t("teamSelector.emptyPlaceholder");
  ...
  const announceSelection = useCallback(
    (teamName: string) => {
      ...
      announcement.textContent = t("teamSelector.selectedAnnounce", {
        name: teamName,
      });
      ...
      return (
        ...
        >
          ...
          <span
            ...
            aria-label={`${formatBalance(team.balanceCents)} ${formatBalanceLabel(team.balanceCents, showCreditsRemaining, t)}`}
          >
            {formatBalance(team.balanceCents)}{" "}
            {formatBalanceLabel(team.balanceCents, showCreditsRemaining, t)}
          </span>
        </CommandItem>
      );
    },
  ...
  return (
    ...
    >
      <Command
        ...
        aria-label={ariaLabel || t("teamSelector.ariaLabel")}
      >
        <CommandInput
          ref={inputRef}
          id={id}
          placeholder={hasTeams ? displayPlaceholder : displayEmptyPlaceholder}
          value={displayValue}
        ...
        {selectedTeam && !listOpen && showCreditsRemaining && (
          ...
          >
            ...
            {formatBalanceLabel(selectedTeam.balanceCents, true, t)}
          </span>
        )}
        <CommandList
          className={cn(listOpen ? "block" : "hidden")}
          role="listbox"
          aria-label={t("teamSelector.listboxAriaLabel")}
        >
          <CommandEmpty>{t("teamSelector.noTeamFound")}</CommandEmpty>
          {selectedTeam && (
            <CommandItem
              key="__clear__"
              value={t("teamSelector.clearSelection")}
              onSelect={handleClear}
              role="option"
            >
              <div className="truncate text-sm">
                {t("teamSelector.clearSelection")}
              </div>
            </CommandItem>
          )}
          {regularTeams.length > 0 && (
            <CommandGroup
              heading={t("teamSelector.teamsHeading")}
              role="group"
              aria-label={t("teamSelector.regularTeamsAriaLabel")}
            >
          ...
          {personalTeams.length > 0 && (
            <CommandGroup
              heading={t("teamSelector.personalHeading")}
              role="group"
              aria-label={t("teamSelector.personalTeamsAriaLabel")}
            >
