...
export function TeamDetailsPanel({
    ...
    async (newName: string) => {
      ...
      startRenameTransition(async () => {
        try {
          ...
          toast.success(t("teamNameUpdated"));
          setEditingName(false);
          setRenameValue(trimmed);
          router.refresh();
        } catch (err: unknown) {
          const message =
            err instanceof Error ? err.message : t("failedToUpdateTeamName");
          toast.error(message);
        }
      });
    },
    [team, currentUserId, router, t]
  );
    ...
    async (newDescription: string) => {
      ...
      startRenameTransition(async () => {
        try {
          ...
          toast.success(t("teamDescriptionUpdated"));
          setEditingDescription(false);
          ...
          const message =
            err instanceof Error
              ? err.message
              : t("failedToUpdateTeamDescription");
          toast.error(message);
          setDescriptionValue(team.description || "");
          setEditingDescription(false);
        }
    ...
    (policy: TeamJoinPolicy) => {
      ...
      startJoinPolicyTransition(async () => {
        try {
          ...
          toast.success(t("teamJoinSettingsUpdated"));
          if (policy === "AUTO_JOIN") {
          ...
          const message =
            err instanceof Error
              ? err.message
              : t("failedToUpdateJoinSettings");
          toast.error(message);
        }
      });
    },
    [team, currentUserId, router, joinPolicyOverride, onJoinPolicyChange, onFetchJoinRequests, t]
  );
  ...
  const joinPolicyOptions = [
    {
      value: "INVITE_ONLY" as TeamJoinPolicy,
      label: t("joinPolicies.inviteOnly.label"),
      description: t("joinPolicies.inviteOnly.description"),
    },
    {
      value: "SECRET" as TeamJoinPolicy,
      label: t("joinPolicies.secret.label"),
      description: t("joinPolicies.secret.description"),
    },
    {
      value: "REQUEST_TO_JOIN" as TeamJoinPolicy,
      label: t("joinPolicies.requestToJoin.label"),
      description: t("joinPolicies.requestToJoin.description"),
    },
    {
      value: "SELF_JOIN" as TeamJoinPolicy,
      label: t("joinPolicies.selfJoin.label"),
      description: t("joinPolicies.selfJoin.description"),
    },
    {
      value: "AUTO_JOIN" as TeamJoinPolicy,
      label: t("joinPolicies.autoJoin.label"),
      description: t("joinPolicies.autoJoin.description"),
    },
  ];
    ...
    <>
      ...
      <div className="mb-6">
        {editingDescription ? (
          ...
          >
            {descriptionValue || t("addDescription")}
          </p>
        )}
      </div>
      ...
      {!team.isPersonal && (
        <div className="mt-6">
          <h3 className="scroll-m-20 text-2xl font-semibold tracking-tight">
            {t("teamJoinPolicyTitle")}
          </h3>
          {team.isDefaultForCompany && (
            <p className="text-muted-foreground mt-2 text-sm">
              {t("defaultTeamPolicyNotice")}
            </p>
          )}
          <RadioGroup
