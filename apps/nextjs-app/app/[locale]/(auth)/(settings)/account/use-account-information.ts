...
export function useAccountInformation({
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!ALLOWED_IMAGE_TYPES.includes(file.type)) {
        toast.error(t("information.errorUnsupportedImage"));
        return;
      }
      if (file.size > MAX_IMAGE_SIZE) {
        toast.error(t("information.errorImageTooLarge"));
        return;
      }
  ...
  }, [draftName, currentName, draftImageFile, removeExistingImage, t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t("information.successUpdate"));
      setIsEditing(false);
    } catch (error) {
      ...
      setSaveError(t("information.errorUpdate"));
      const errorMessage =
        error instanceof Error ? error.message : t("information.errorUpdate");
