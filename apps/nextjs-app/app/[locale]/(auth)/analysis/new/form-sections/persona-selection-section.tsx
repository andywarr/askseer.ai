...
export function PersonaSelectionSection({
  ...
  return (
    <div>
      <FormLabel className="mb-2 block">{tc("targetUserLabel")}</FormLabel>

      {/* Selected persona cards */}
      {selectedPersonas.length > 0 && (
        <div className="mb-3 flex flex-col gap-2">
          {selectedPersonas.map((p) => {
            const name = p.persona?.name || p.name || tc("unnamedPersona");
      ...
      {isPersonasLoading ? (
        ...
        <PersonaSelect
          ...
          placeholder={tc("targetUserPlaceholderSearch")}
