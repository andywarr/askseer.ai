...
import { MediaPlayer } from "@/apps/nextjs-app/app/[locale]/(auth)/analysis/[id]/media-player";
...
const IMPACT_LEVEL_KEYS: Record<number, string> = {
  1: "impactLevels.low",
  2: "impactLevels.moderate",
  3: "impactLevels.significant",
  4: "impactLevels.high",
  5: "impactLevels.critical"
};
...
function ImpactBadge({
  ...
  if (!canEdit || !onSelect) {
    return <Badge className={level.badgeColor}>{t(IMPACT_LEVEL_KEYS[level.value] as any)}</Badge>;
  }
      ...
      <PopoverTrigger asChild>
        ...
        >
          ...
          >
            ...
            {t(IMPACT_LEVEL_KEYS[level.value] as any)}
          </Badge>
        </div>
      </PopoverTrigger>
      <PopoverContent
              ...
              {IMPACT_LEVELS.map((impactLevel) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span>{t(IMPACT_LEVEL_KEYS[impactLevel.value] as any)}</span>
                  </div>
  ...
  const handleSaveNewInsight = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        toast.success(t("insightCreated"));
        resetNewInsightDrawer();
      } else if (!result.success) {
        toast.error(result.error || t("failedCreateInsight"));
      }
    } catch {
      toast.error(t("failedCreateInsight"));
    } finally {
  ...
  const handleAddQuote = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        toast.error(result.error || t("failedAddQuote"));
      }
    } catch {
      toast.error(t("failedAddQuote"));
    } finally {
    ...
    async (quoteId: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.success(t("quoteRemoved"));
        } else {
          toast.error(result.error || t("failedRemoveQuote"));
        }
      } catch {
        toast.error(t("failedRemoveQuote"));
      } finally {
        setDeletingQuoteId(null);
      }
    },
    [deleteQuote, userId, studyId, t],
  );
    ...
    async (insightId: string, tag: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.error || t("failedAddTag"));
        }
      } catch {
        toast.error(t("failedAddTag"));
      } finally {
        setSavingTag(false);
      }
    },
    [addTag, userId, studyId, t],
  );
    ...
    async (tagId: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.error || t("failedRemoveTag"));
        }
      } catch {
        toast.error(t("failedRemoveTag"));
      } finally {
        setRemovingTagId(null);
      }
    },
    [removeTag, userId, studyId, t],
  );
    ...
    async (insightId: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.success(t("insightRemoved"));
        } else {
          toast.error(result.error || t("failedRemoveInsight"));
        }
      } catch {
        toast.error(t("failedRemoveInsight"));
      } finally {
        setDeletingInsightId(null);
      }
    },
    [deleteInsight, userId, studyId, t],
  );
    ...
    async (
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.success(t("insightUpdated"));
        } else {
          toast.error(result.error || t("failedUpdateInsight"));
        }
      } catch {
        toast.error(t("failedUpdateInsight"));
      } finally {
        setSavingField(null);
      }
    },
    ...
    <div>
                    ...
                    {selectedImpacts.length <= 2 ? (
                      selectedImpacts.map((level) => {
                        ...
                        return (
                          ...
                          >
                            {info ? t(IMPACT_LEVEL_KEYS[info.value] as any) : ""}
                          </Badge>
                        );
                      })
                    ) : (
                      <Badge
                        ...
                        className="rounded-sm px-1 font-normal"
                      >
                        {t("selected", { count: selectedImpacts.length })}
                      </Badge>
                    )}
                  </div>
                </>
                  ...
                  {IMPACT_LEVELS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span>{t(IMPACT_LEVEL_KEYS[option.value] as any)}</span>
                      </CommandItem>
                    );
                  })}
                </CommandGroup>
                ...
                {editingTitleId === insight.id ? (
                  ...
                  >
                    <span className="text-left font-bold text-base">
                      <TranslationWrapper text={getInsightValue(insight, "title")} sourceLocale={studyLocale} inline />
                    </span>
                    {canEdit && (
                      <>
                        <InlineActionButton
              ...
              <div className="flex flex-col gap-4">
                  ...
                  return hasQuotes || (canEdit && sourceFiles.length > 0) ? (
                    <div>
                      <h5 className="mb-2 text-sm font-semibold">
                        {t("quotes").charAt(0).toUpperCase() + t("quotes").slice(1)}
                      </h5>
                      <div className="relative overflow-hidden">
                        ...
                        {allQuotes.map((q) => (
                          ...
                          >
                            ...
                            >
                              &ldquo;
                              <TranslationWrapper text={q.quote} sourceLocale={studyLocale} inline />
                              &rdquo;
                            </p>
                        ...
                        {canEdit && sourceFiles.length > 0 && (
                          ...
                          >
                            ...
                            <span className="text-xs font-medium">{t("addQuotes")}</span>
                          </button>
                        )}
                        </div>
                      </div>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {insight.theme && (
                    ...
                    >
                      <TranslationWrapper text={insight.theme} sourceLocale={studyLocale} inline />
                    </Badge>
                  )}
                  {insight.tags
                    ...
                    .map((tag) => (
                      ...
                      >
                        <TranslationWrapper text={tag.tag} sourceLocale={studyLocale} inline />
                        {canEdit && !isMobile && (
                          <span
                            role="button"
      ...
      >
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            <DialogTitle>{t("deleteInsightConfirmTitle")}</DialogTitle>
            <DialogDescription>
              {t("deleteInsightConfirmDesc")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DrawerContent direction="right">
          <DrawerHeader className="shrink-0 border-b px-4 py-3">
            <DrawerTitle>
              {isViewQuoteMode ? t("viewQuotes") : t("addQuotes")}
            </DrawerTitle>
            <DrawerDescription>
              {isViewQuoteMode
                ? t("viewingQuoteContext")
                : t("selectQuotesContext")}
            </DrawerDescription>
          </DrawerHeader>
                    ...
                    <div className="flex min-h-0 flex-1 flex-col gap-1.5 px-4 py-3">
                        ...
                        onMouseUp={(e) => {
                          ...
                          if (
                            ...
                            if (newComposed.length <= MAX_QUOTE_LENGTH) {
                              ...
                              toast.error(
                                t("selectionExceedLimit", { max: MAX_QUOTE_LENGTH, len: newComposed.length }),
                              );
                            }
                            selection.removeAllRanges();
                      ...
                      <div className="mt-1.5 flex shrink-0 flex-wrap gap-3 text-[11px]">
                        <span className="flex items-center gap-1">
                          ...
                          <span className="text-zinc-500">{t("thisInsight")}</span>
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="inline-block h-2.5 w-2.5 rounded-sm bg-purple-100" />
                          <span className="text-zinc-500">{t("otherInsights")}</span>
                        </span>
                        {!isViewQuoteMode && (
                          <span className="flex items-center gap-1">
                            <span className="inline-block h-2.5 w-2.5 rounded-sm bg-blue-100" />
                            <span className="text-zinc-500">{t("yourSelection")}</span>
                          </span>
                        )}
                      </div>
                    </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-medium text-blue-700">
                              {quoteSegments.length === 1
                                ? t("selectedQuote")
                                : t("selectionsCount", { count: quoteSegments.length })}
                            </span>
                          </div>
                          <button
                            type="button"
      ...
      >
        <DrawerContent direction="right">
          <DrawerHeader className="shrink-0 border-b px-4 py-3">
            <DrawerTitle>
              {newInsightStep === "quotes" ? t("addQuotes") : t("describeInsight")}
            </DrawerTitle>
            <DrawerDescription>
              {newInsightStep === "quotes"
                ? t("selectQuotesSupportInsight")
                : t("fillInsightDetails")}
            </DrawerDescription>
            {/* Step indicator */}
                            ...
                            onMouseUp={(e) => {
                              ...
                              if (
                                ...
                                if (newComposed.length <= MAX_QUOTE_LENGTH) {
                                  ...
                                  toast.error(
                                    t("selectionExceedLimitSimple", { max: MAX_QUOTE_LENGTH }),
                                  );
                                }
                                selection.removeAllRanges();
                              } else if (!(e.metaKey || e.ctrlKey)) {
                    ...
                    <div className="flex shrink-0 flex-col gap-2.5 border-t px-4 py-3">
                      {niQuoteSegments.length > 0 ? (
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs font-medium text-blue-700">
                                {niQuoteSegments.length === 1
                                  ? t("selectedQuote")
                                  : t("selectionsCount", { count: niQuoteSegments.length })}
                              </span>
                            </div>
                            <button
                              type="button"
                        ...
                        <div className="flex items-center gap-1.5 rounded-md border border-dashed border-zinc-200 bg-zinc-50 px-3 py-2">
                          <Quote className="h-3.5 w-3.5 text-zinc-300" />
                          <span className="text-xs text-zinc-400">
                            {t("highlightTextInstructionsNewInsight")}
                          </span>
                        </div>
                      )}
                      <div className="flex items-center gap-2">
                    ...
                    {newInsightQuotes.length > 0 && (
                      <div className="shrink-0 border-t px-4 py-3">
                        <p className="mb-2 text-xs font-medium text-zinc-500">
                          {t("quotesAddedCount", { count: newInsightQuotes.length, plural: newInsightQuotes.length === 1 ? "" : "s" })}
                        </p>
              ...
              <div className="flex flex-1 flex-col gap-4 overflow-y-auto px-4 py-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-zinc-500">
                    {t("titleRequired").replace("*", "").trim()} <span className="text-red-500">*</span>
                  </label>
                  <Input
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-zinc-500">
                    {t("insightStatementRequired").replace("*", "").trim()} <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    ...
                    placeholder={t("insightStatementPlaceholder")}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    placeholder={t("implicationPlaceholder")}
                    className="w-full resize-none rounded-md border border-zinc-300 px-3 py-2 text-sm focus:border-zinc-400 focus:outline-none"
                    rows={3}
                  />
                </div>
                ...
                {newInsightQuotes.length > 0 && (
                  <div className="rounded-md border bg-zinc-50 p-3">
                    <p className="mb-1.5 text-xs font-medium text-zinc-500">
                      {newInsightQuotes.length} {newInsightQuotes.length === 1 ? t("supportingQuote") : t("supportingQuotes")}
                    </p>
