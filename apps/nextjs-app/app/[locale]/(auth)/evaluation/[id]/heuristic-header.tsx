...
function HeuristicHeaderComponent({
  ...
  const sourceKeys: Record<SourceFilterValue, string> = {
    AI: "ai",
    HUMAN: "human",
    AI_HUMAN: "hybrid",
  };
    ...
    <div className="mb-4 flex flex-col gap-4">
        ...
        <div className="flex shrink-0 flex-col items-end gap-4 sm:flex-row sm:items-baseline sm:gap-4">
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="hidden cursor-help items-baseline gap-1 sm:flex">
                ...
                <span className={gradeInfo.colorClass}>{t("grade")}</span>
              </span>
            </TooltipTrigger>
            <TooltipContent className="max-w-xs p-0">
              <div className="p-3">
                <p className="mb-2 text-sm font-semibold">
                  {t("qualityScore", { score: gradeInfo.qualityScore ?? 0 })}
                </p>
          ...
          <span className="hidden items-baseline gap-1 sm:flex">
            ...
            <span className="text-zinc-500">
              {t("issueLabel", { count: totalIssues })}
            </span>
          </span>
          <span
          ...
          >
            ...
            <span>{t("violationLabel", { count: violatedCount })}</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        ...
        >
          ...
          <span className="hidden sm:inline">{t("onlyShowViolated")}</span>
        </Button>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="secondary" size="sm" className="h-8 gap-1.5">
                ...
                <span className="hidden sm:inline">{t("screens")}</span>
                {selectedScreens.length < presignedUrls.length && (
                  <>
                    ...
                    >
                      {t("selectedCount", { count: selectedScreens.length })}
                    </Badge>
                  </>
                )}
              </Button>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="secondary" size="sm" className="h-8 gap-1.5">
              ...
              <span className="hidden sm:inline">{t("heuristic")}</span>
              {selectedHeuristicIds.length > 0 && (
                <>
                  <span className="mx-1 h-4 w-px bg-zinc-300 dark:bg-zinc-600" />
                  <Badge
                    ...
                    className="rounded-sm px-1 font-normal"
                  >
                    {t("selectedCount", { count: selectedHeuristicIds.length })}
                  </Badge>
                </>
              )}
            </Button>
          ...
          <PopoverContent className="w-[280px] p-0" align="start">
            <Command>
              <CommandInput
                placeholder={t("searchHeuristics")}
                value={heuristicSearch}
                onValueChange={setHeuristicSearch}
              />
              <CommandList>
                <CommandEmpty>{t("noHeuristicsFound")}</CommandEmpty>
                <CommandGroup>
                  {heuristicOptions
                    .filter((h) =>
                      h.name
                  ...
                  <div className="flex gap-1">
                    {selectedSeverities.length <= 2 ? (
                      ...
                      <Badge
                        ...
                        className="rounded-sm px-1 font-normal"
                      >
                        {t("selectedCount", { count: selectedSeverities.length })}
                      </Badge>
                    )}
                  </div>
                </>
                  ...
                  {SEVERITY_FILTER_OPTIONS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span>{tSeverity(severityKeys[option.value])}</span>
                      </CommandItem>
                    );
                  })}
                </CommandGroup>
                  ...
                  {SOURCE_FILTER_OPTIONS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{tSource(sourceKeys[option.value])}</span>
                      </CommandItem>
                    );
                  })}
                </CommandGroup>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="secondary" size="sm" className="h-8 gap-1.5">
                ...
                <span className="hidden sm:inline">
                  {sortBy === "heuristic" ? t("heuristic") : capitalize(t("violationLabel", { count: 2 }))}
                </span>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                ...
                {capitalize(t("violationLabel", { count: 2 }))}
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuLabel className="text-xs font-normal text-zinc-500">
                {t("sortDirection")}
