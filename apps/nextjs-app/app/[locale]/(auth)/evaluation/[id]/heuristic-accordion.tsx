...
interface HeuristicAccordionProps {
  ...
  sortedEntries?: [string, HEResultData[]][];
  ...
  const handleAddIssue = useCallback(
    async (stepIndex: number, description: string, severity: number) => {
      ...
      try {
        ...
        if (!result.success) {
          toast.error(result.error || t("failedAddIssue"));
          return;
        }
        ...
        setAddDialogOpen((prev) => ({
          ...prev,
          [selectedHeuristicKey]: false,
        }));
        setSelectedHeuristicKey(null);

        toast.success(t("successAddIssue"));
      } catch (error) {
        toast.error(t("failedAddIssue"));
      }
    },
    [
      canManage,
  ...
  return (
    <>
      ...
      >
        {(
          sortedEntries ??
          Object.entries(groupedResults).sort(([, itemsA], [, itemsB]) => {
          ...
          return (
            <AccordionItem key={key} value={key}>
                      ...
                      <div className="flex gap-4 text-sm text-zinc-500">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs">
                            {t("screenLabel", { count: stepsCount })}
                          </span>
                        </div>
                        <div className="flex flex-col">
                          <span className="font-semibold">{issuesCount}</span>
                          <span className="text-xs">
                            {t("issueLabel", { count: issuesCount })}
                          </span>
                        </div>
                        <div className="flex flex-col">
                          <span className="font-semibold">
                          ...
                          <span className="text-xs">
                            {t("recommendationLabel", { count: recommendationsCount })}
                          </span>
                        </div>
                      </div>
                    )}
              ...
              <AccordionContent>
                {!isViolated ? (
                  <p className="py-4 text-zinc-500">{t("noIssuesFound")}</p>
                ) : (
