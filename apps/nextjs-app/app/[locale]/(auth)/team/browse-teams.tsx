...
import { TeamCard } from "@/apps/nextjs-app/app/[locale]/(auth)/team/team-card";
import type { Team } from "@/apps/nextjs-app/app/[locale]/(auth)/team/team-card";
...
export default function BrowseTeams({ teams, currentUserId }: Props) {
  const t = useTranslations("BrowseTeams");
    ...
    (teamId: string) => {
      ...
      startTransition(async () => {
        try {
          ...
          toast.success(t("successJoined"));
          router.refresh();
        } catch (err) {
          const message =
            err instanceof Error ? err.message : t("failedToJoin");
          toast.error(message);
        } finally {
    ...
    (teamId: string, note?: string) => {
      ...
      startTransition(async () => {
        try {
          ...
          toast.success(t("successRequested"));
          router.refresh();
        } catch (err) {
          const message =
            err instanceof Error ? err.message : t("failedToRequest");
          toast.error(message);
        } finally {
