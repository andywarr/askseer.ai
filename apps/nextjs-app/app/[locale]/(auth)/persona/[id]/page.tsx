...
import { PersonaVersionCard } from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-version-card";
import { PersonaRelatedStudies } from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-related-studies";
import { UserMetadataDisplay } from "@/apps/nextjs-app/components/study/user-metadata";
import { PersonaSectionCard } from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-section-card";
import { PersonaHeader } from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-header";
import { PersonaErrorBoundary } from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-error-boundary";
...
} from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-skeletons";
...
} from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-utils";
...
import type {
  ...
  AssociatedStudy,
  EvaluationEntry,
} from "@/apps/nextjs-app/app/[locale]/(auth)/persona/[id]/persona-types";
...
export default async function Page(props: { params: Promise<{ id: string; locale: string }> }) {
  ...
  const [study, bookmarkedStudyIds, shareInfo, t, tMeta, tFields] = await Promise.all([
    getPersona(id, session.userId),
    getBookmarkedStudyIds(session.userId),
    getStudyShareInfo(id, session.userId),
    getTranslations({ locale, namespace: "PersonaDetail" }),
    getTranslations({ locale, namespace: "StudyMetadata" }),
    getTranslations({ locale, namespace: "StudyWizardForms.persona.fields" }),
  ]);
    ...
    <TranslationProvider studyId={id}>
      <PersonaErrorBoundary>
        <div className="w-full">
                        ...
                        <div className="min-w-0">
                          ...
                          {item.isList ? (
                            ...
                            <div className="leading-6 font-medium break-words">
                              <TranslationWrapper text={item.value} sourceLocale={studyLocale} inline />
                            </div>
                          )}
                        </div>
                      </div>
          ...
          {/* Behaviors */}
          {(() => {
            const items = buildBehaviorsItems(persona.behaviors || {}, {
              Cpu,
              Smartphone,
              MessageSquare,
              Zap,
              ...
              <div className="overflow-x-auto pb-2">
                <div className="flex gap-4">
                  {personaVersions.map((version) => {
                    const photoUrl =
                      versionPhotoMap.get(version.id) ?? undefined;
