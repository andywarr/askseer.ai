...
function PersonaVersionCardComponent({
  ...
  const t = useTranslations("PersonaVersionCard");
  const updatedAtFormatted = formatDateTime(version.study.updatedAt, locale);
        ...
        <CardHeader className={cn("relative h-40", imageClassName)}>
          {photoUrl ? (
            <Image
              ...
              src={photoUrl}
              fill
              alt={t("imageAlt", { name: version.name || t("untitled"), version: version.version })}
              unoptimized
            />
          ) : (
