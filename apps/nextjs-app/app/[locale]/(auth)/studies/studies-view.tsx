...
export function StudiesView({
  ...
  const getTranslatedStudyTypeLabel = useCallback((type: StudyType) => {
    switch (type) {
  ...
  const columns: ColumnDef<StudyWithPreview>[] = useMemo(
    () => [
      ...
      {
        id: "name",
        accessorFn: (row) => row.study.name || t("untitled"),
            ...
            <div className="flex items-center gap-2">
              ...
              {isPending && (
                <Tooltip>
                  ...
                  <TooltipContent>{t("tooltips.processing")}</TooltipContent>
                </Tooltip>
              )}
              {isFailed && (
                <Tooltip>
                  ...
                  <TooltipContent>{t("tooltips.failed")}</TooltipContent>
                </Tooltip>
              )}
            </div>
          );
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-zinc-600 dark:text-zinc-400">
            {getTranslatedStudyTypeLabel(row.original.study.type)}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-zinc-600 dark:text-zinc-400">
            {formatDate(row.original.study.createdAt, locale)}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-zinc-600 dark:text-zinc-400">
            {formatDate(row.original.study.updatedAt, locale)}
          </span>
        ),
      },
      {
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  ...
                  <span className="sr-only">{t("tooltips.openMenu")}</span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent
                align="end"
                    ...
                    <TooltipTrigger asChild>
                      <span className="w-full">
                        <DropdownMenuItem disabled={true}>
                          ...
                          <span className="text-zinc-400">{t("actions.delete")}</span>
                        </DropdownMenuItem>
                      </span>
                    </TooltipTrigger>
                    <TooltipContent side="left">
                      <p>{t("tooltips.noDeletePersona")}</p>
                    </TooltipContent>
                  </Tooltip>
                )}
              </DropdownMenuContent>
  ...
  const filteredStudies = useMemo(() => {
    return studies.filter(({ study }) => {
      ...
      if (searchQuery.trim()) {
        ...
        const type = getTranslatedStudyTypeLabel(study.type).toLowerCase();
    ...
    <div className="space-y-4">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        >
          ...
          />
          <span className="hidden sm:inline">{t("filters.bookmarked")}</span>
        </Button>
        ...
        <Popover open={typePopoverOpen} onOpenChange={setTypePopoverOpen}>
          <PopoverTrigger asChild>
            <Button variant="secondary" size="sm" className="h-8 gap-1.5">
              ...
              <span className="hidden sm:inline">{t("filters.type")}</span>
                  ...
                  <div className="flex gap-1">
                    {selectedTypes.length <= 2 ? (
                      selectedTypes.map((type) => (
                        ...
                        >
                          {getTranslatedStudyTypeLabel(type)}
                        </Badge>
                      ))
                    ) : (
                      <Badge
                        ...
                        className="rounded-sm px-1 font-normal"
                      >
                        {t("filters.selectedCount", { count: selectedTypes.length })}
                      </Badge>
                    )}
                  </div>
                </>
                  ...
                  {STUDY_TYPE_OPTIONS.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{getTranslatedStudyTypeLabel(option.value)}</span>
                      </CommandItem>
                    );
                  })}
                </CommandGroup>
          ...
          <Popover open={ownerPopoverOpen} onOpenChange={setOwnerPopoverOpen}>
            <PopoverTrigger asChild>
              <Button variant="secondary" size="sm" className="h-8 gap-1.5">
                ...
                <span className="hidden sm:inline">{t("filters.owner")}</span>
                {selectedOwnerIds.length > 0 && (
                  <>
                    <span className="mx-1 h-4 w-px bg-zinc-300 dark:bg-zinc-600" />
                    <div className="flex gap-1">
                      {selectedOwnerIds.length <= 2 ? (
                        ...
                        <Badge
                          ...
                          className="rounded-sm px-1 font-normal"
                        >
                          {t("filters.selectedCount", { count: selectedOwnerIds.length })}
                        </Badge>
                      )}
                    </div>
                  </>
