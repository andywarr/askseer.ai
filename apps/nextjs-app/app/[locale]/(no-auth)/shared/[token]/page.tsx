...
// Components imports
import { SharedHeuristicEvaluation } from "@/apps/nextjs-app/app/[locale]/(no-auth)/shared/[token]/shared-heuristic-evaluation";
import { SharedCognitiveWalkthrough } from "@/apps/nextjs-app/app/[locale]/(no-auth)/shared/[token]/shared-cognitive-walkthrough";
import { SharedPersonaView } from "@/apps/nextjs-app/app/[locale]/(no-auth)/shared/[token]/shared-persona-view";
import { SharedAnalysisInsightsClient } from "@/apps/nextjs-app/app/[locale]/(no-auth)/shared/[token]/shared-analysis-insights-client";
import { SharedTakeaways } from "@/apps/nextjs-app/app/[locale]/(no-auth)/shared/[token]/shared-takeaways";
...
export async function generateMetadata(props: {
  ...
  const tShared = await getTranslations({ locale, namespace: "SharedStudy" });
  return {
    title: `${tShared("sharedStudy")} | Seer`,
    description: tShared("ctaDesc"),
  };
}
...
export default async function SharedStudyPage(props: {
  ...
  const createdByName = study.createdByUser?.name || tMeta("unknown");
  ...
  const getStudyTypeLabel = (type: StudyType): string => {
    switch (type) {
      case StudyType.COGNITIVE_WALKTHROUGH:
        return tShared("typeLabel.walkthrough");
      case StudyType.HEURISTIC_EVALUATION:
        return tShared("typeLabel.evaluation");
      case StudyType.PERSONA:
        return tShared("typeLabel.persona");
      case StudyType.QUAL_ANALYSIS:
        return tShared("typeLabel.analysis");
      default:
        return tShared("typeLabel.study");
    }
  };
  ...
  if (study.type === StudyType.PERSONA && study.persona) {
    return (
      <TranslationProvider studyId={study.id}>
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-4">
                ...
                <Button asChild variant="default" size="sm">
                  <Link href="/signin">{tShared("signin")}</Link>
                </Button>
              </div>
            </div>
          </header>
          ...
          <footer className="border-t border-zinc-200 bg-white py-8 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8">
              <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                {tShared("wantUncoverInsights")}
              </h2>
              <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
                {tShared("ctaDesc")}
              </p>
              <Button asChild className="mt-4">
                <Link href="/signin">{tShared("getStartedFree")}</Link>
              </Button>
            </div>
          </footer>
        </div>
        ...
        <div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="max-w-40 truncate text-xs text-zinc-600 dark:text-zinc-400">
                          {file.originalName || tShared("untitled")}
                        </span>
                      </a>
                    );
                  })}
                    ...
                    <div className="mt-1 flex items-center gap-3">
                      ...
                      <div className="flex min-w-0 flex-col">
                        <span className="truncate leading-5 font-medium">
                          {personaName ?? tMeta("unnamedPersona")}
                        </span>
                        <span className="truncate leading-5 text-zinc-600">
                          {personaDescription ?? tMeta("noDescription")}
                        </span>
                      </div>
                    </div>
                  ) : (
          ...
          {(study as any).tldrStatus === "COMPLETED" &&
            (study as any).takeaways?.length > 0 && (
              <SharedTakeaways
                takeaways={(study as any).takeaways}
