...
export function MemberSelector({
  ...
  const filteredAvailableMembers = availableMembers.filter((m) => !selectedMembers[m.userId]);
  ...
  const getMemberDisplay = (member: CompanyMember) => member.user.name || member.user.email;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <Select
                  value={role}
                  onValueChange={(value) => onRoleChange(userId, value as MemberRole)}
                  disabled={disabled}
                >
                  <SelectTrigger 
  ...
  const renderAddMemberUI = () => {
      ...
      if (!hasSelectedMembers) {
        return (
          <p className="mb-4 text-sm text-orange-500" role="alert">
            There are no company members available to add.
          </p>
        );
      }
      return null;
    ...
    return (
      <div className="mb-4 flex items-start gap-2" role="group" aria-label={ariaLabel}>
        <div
          className="flex-1"
          onBlur={(e) => {
