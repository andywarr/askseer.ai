...
export async function POST(request: Request) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (!teamId || !purchasedByUserId || !credits || credits < 1) {
    ...
    // Only add credits if payment was successful
    if (session.payment_status === "paid") {
      try {
        ...
        logger.info("Successfully processed credit purchase", {
          sessionId: session.id,
          teamId,
          credits,
        });
      } catch (error) {
        logger.error("Failed to add credits after successful payment", {
          sessionId: session.id,
          teamId,
          credits,
          error,
