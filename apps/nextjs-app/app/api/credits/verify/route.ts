...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!teamId || !purchasedByUserId || !credits) {
    ...
    // Add credits immediately (idempotent - webhook may also add them)
    try {
      ...
      logger.info("Credits added via verify endpoint", {
        sessionId,
        teamId,
        credits,
      });
      ...
      // If credits were already added by webhook, this might fail
      // Check if it's a duplicate issue
      logger.warn("Failed to add credits in verify endpoint", {
        sessionId,
        error,
        note: "Webhook may have already processed this",
      });

      // Return success anyway - credits may already be added
      return NextResponse.json({
        success: true,
        paymentStatus: session.payment_status,
        credits,
        note: "Credits may have been added by webhook",
      });
    }
  } catch (error) {
