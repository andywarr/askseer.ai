...
export function InfoCard({
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      toast.success(t("successUpdate", { type: t(`${type}Label`, { count: 1 }) }));
    } catch (error) {
      console.error(`Error updating ${type}:`, error);
      toast.error(t("failedUpdate", { type: t(`${type}Label`, { count: 1 }) }));
  ...
  const handleDeleteClick = async () => {
    ...
    try {
      ...
      onDelete?.();
      router.refresh(); // Refresh server component to update study metadata
      toast.success(t("successDelete", { type: t(`${type}Label`, { count: 1 }) }));
    } catch (error) {
      console.error(`Error deleting ${type}:`, error);
      toast.error(t("failedDelete", { type: t(`${type}Label`, { count: 1 }) }));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleFeedbackClick = async (feedbackType: "up" | "down") => {
    ...
    try {
      ...
      toast.error(t("failedSaveFeedback"));
    } finally {
  ...
  const handleSeverityChange = async (newSeverity: SeverityRating) => {
    ...
    try {
      ...
      toast.error(t("failedUpdate", { type: t("severity").toLowerCase() }));
      ...
      >
        <div className={!isMobile ? "pr-8 print:pr-0" : ""}>
          {isEditing ? (
            ...
            <TranslationWrapper text={editedContent} sourceLocale={studyLocale} />
          )}
        </div>
