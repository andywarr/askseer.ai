...
function toSentence(g: Goal): string {
  const want = (g.want || "").trim();
  const soThat = (g.soThat || "").trim();
  if (!want && !soThat) return "";
  if (want && soThat) return `I want to ${want} so that I can ${soThat}`;
  // We typically require both, but be defensive
  if (want) return `I want to ${want}`;
  return soThat;
}

function normalize(initial: unknown): string[] {
  if (Array.isArray(initial)) {
    ...
    return (initial as Goal[])
      .map((g) => toSentence({ want: g.want || "", soThat: g.soThat || "" }))
      .filter(Boolean);
  }
...
export function GoalsEditor({
  value,
  onChange,
  placeholderWant = "do X",
  placeholderSoThat = "achieve Y",
  rightAction,
}: {
  value: unknown;
  ...
  const add = () => {
    ...
    if (!w && !s) return;
    const sentence = toSentence({ want: w, soThat: s });
