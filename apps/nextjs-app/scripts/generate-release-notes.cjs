...
async function generateWeekNotesWithLlm({
  ...
  const basePromptParts = [
    `You are an expert Product Manager writing release notes for ${weekLabel}.`,
    "You are given ALL commits from this week.",
    "Your job is to look ACROSS commits and produce concise, user-facing release notes that focus on VALUE.",
    "\nRELEASE NOTES GENERATION RULES:",
    "\n1. STRICT PROHIBITIONS (The 'Dirty Laundry' List):",
    "- NO internal paths or file extensions (e.g., never mention 'src/', '/api/trpc', '.tsx', 'schema.prisma').",
    "- NO vendor names or infrastructure details (e.g., never mention 'AWS', 'Prisma', 'Redis', 'Vercel', 'Docker'). Users don't care how it's built.",
    "- NO 'refactor', 'cleanup', 'chore', 'bump', 'lint', 'fix typo'. If it's not a user-facing change, IGNORE IT.",
    "- NO technical jargon like 'endpoints', 'payloads', 'migrations', 'database columns'.",
    "- NO mention of 'timeouts', 'limits', 'retries', or 'errors' unless it's a resolved bug that users reported.",
    "- NO secrets, tokens, or PII.",
    "\n2. WRITING GUIDELINES:",
    "- Focus on the USER BENEFIT. Instead of 'Added sort to table', say 'Users can now sort the team list'.",
    "- AGGREGATE related commits. If there are 5 commits about 'search', write ONE highlight: 'Improved search accuracy and performance'.",
    "- Use a professional, polished tone. No emojis, no exclamation marks.",
    "- If a feature is 'internal only' or 'dev facing', OMIT IT entirely.",
    "\n3. OUTPUT FORMAT:",
    "- Combine features + fixes into one list called 'highlights'.",
    "- Output must be ONLY valid JSON.",
    "\nReturn schema:",
    '{"summary":"1-2 sentences summarizing the main theme of the week","highlights":[{"type":"feat"|"fix","text":"User-focused description..."}]}',
        ...
        input: [
          {
            role: "system",
            content:
              "You are an expert Product Manager. You write release notes that focus on value and hide implementation details. Output STRICT JSON only.",
          },
          { role: "user", content: prompt },
        ],
      }),
