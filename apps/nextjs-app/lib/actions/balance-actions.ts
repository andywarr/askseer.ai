...
const MAX_TRANSFER_AMOUNT_CENTS = 50000000; // $500,000 max transfer
...
async function chargePaymentMethod(params: {
  ...
  const { customerId, paymentMethodId, amountCents, teamId, teamName } = params;
  ...
  try {
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      off_session: true,
      confirm: true,
      description: `Auto-refill: $${(amountCents / 100).toFixed(2)} for ${teamName}`,
      metadata: {
        teamId,
...
export async function transferBalance(
  ...
  const { fromTeamId, toTeamId, amountCents } = params;
  ...
  if (fromTeamId === toTeamId) {
    return actionError("Cannot transfer funds to the same team.");
  }

  if (!Number.isFinite(amountCents) || amountCents < 1) {
    return actionError("Enter a valid amount to transfer.");
  }

  if (amountCents > MAX_TRANSFER_AMOUNT_CENTS) {
    return actionError(
      `Cannot transfer more than $${(MAX_TRANSFER_AMOUNT_CENTS / 100).toFixed(2)} at once.`,
    );
  }
  ...
  try {
    ...
    if (!domainInfo?.company) {
      return actionError(
        "Balance transfers are only available for company members.",
      );
    }
    ...
    if (!hasFromAccess) {
      return actionError(
        "You do not have permission to transfer funds from this team.",
      );
    }

    if (!hasToAccess) {
      return actionError(
        "You do not have permission to transfer funds to this team.",
      );
    }

    // Check the source team has enough balance
    ...
    if ((fromTeam.balanceCents ?? 0) < amountCents) {
      return actionError(
        `Insufficient funds. The source team only has $${((fromTeam.balanceCents ?? 0) / 100).toFixed(2)}.`,
      );
    }
    ...
    logger.info("Balance transferred between teams", {
      userId,
      fromTeamId,
      toTeamId,
      amountCents,
    ...
    logger.error("Error transferring balance", { error, params });
    return actionError(
...
export async function createCheckoutSessionForPaymentSetup(
  ...
  try {
    ...
    const checkoutSession = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/funds?setup_success=true&team=${teamId}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/funds?setup_cancelled=true&team=${teamId}`,
      metadata: {
        teamId,
        userId: user.id,
      },
...
export async function triggerAutoRefill(
  ...
  try {
    ...
    // autoRefillAmount is already in cents
    const refillAmountCents = team.autoRefillAmount;

    // Charge the saved payment method
    const chargeResult = await chargePaymentMethod({
      customerId: team.stripeCustomerId,
      paymentMethodId: team.stripePaymentMethodId,
      amountCents: refillAmountCents,
      teamId,
      teamName: team.name,
    });
    ...
    return actionSuccess({ triggered: true, balanceCents: refillAmountCents });
  } catch (error) {
